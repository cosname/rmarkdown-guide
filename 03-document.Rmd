...
很多 LaTeX 选项都可以通过 YAML 头部传递给 PDF 编译工具。
...
在这个文档中，首先通过 `\documentclass{}` 指定了文档的类，然后在序言部分（`preamble`）导入一些 LaTeX 的包（组件）和进行必要的设置，接下来在 `\begin{document}` 后开始文档的主体内容。
...
一个常见的应用是在序言部分加入自定义 LaTeX 命令或环境，例如下面的 LaTeX 命令可以为正文中的每一个超链接的链接地址添加脚注：
...
将上面的代码保存到 `preamble.tex` 文件中，用 `in_header` 选项将其添加到序言部分：
...
```{r latex-vars, echo=FALSE}
knitr::kable(
  data.frame(
    变量名称 = c("lang", "fontsize", "documentclass", "classoption", "geometry", "mainfont, sansfont, monofont, mathfont", "linkcolor, urlcolor, citecolor"),
    说明 = c("文档的语言代码", "文字大小 （如 10pt，11pt，12pt）", "LaTeX 文档类型 （如 article）", "文档类型的选项（如 oneside）", "页边距（如 margin=1in）", "文档的字体（仅在使用 xelatex 和 lualatex 时可用）", "内部链接、外部链接和引文链接的颜色")
  ),
  caption = "在 YAML 配置中可用的 LaTeX 变量"
)
```
...
```{r rmd-presentation-popup, echo=FALSE, fig.cap="新建 R Markdown 时可选的幻灯片格式",fig.align='center', out.width = "90%"}
knitr::include_graphics("images/rmd-presentation-popup.png", dpi = NA)
```
...
```{r ioslides-presentation-screenshot, echo=FALSE, fig.cap="新建文件编译后生成的 ioslides 幻灯片",fig.align='center', out.width = "90%"}
knitr::include_graphics("examples/ioslides-presentation-screenshot.png", dpi = NA)
```
...
```{r slidy-presentation-screen-shot, echo=FALSE, fig.width=6,  fig.cap="Slidy 幻灯片", fig.align='center', out.width = "90%"}
knitr::include_graphics("examples/slidy-presentation-screenshot.png", dpi = NA)
```
...
Beamer 幻灯片编译后的文件是一个 PDF 文档，图 \@ref(fig:beamer-presentation-screenshot)）。

```{r beamer-presentation-screenshot, echo=FALSE,fig.cap="Beamer 幻灯片示例",fig.align='center', out.width = "90%"}
knitr::include_graphics("examples/beamer-presentation-screenshot.png", dpi = NA)
```
...
这个主题为标题幻灯片添加了页眉和页脚，显示作者、主题、日期和页码等信息，如图 \@ref(fig:beamer-presentation-theme) 所示。

```{r beamer-presentation-theme, echo=FALSE, fig.align='center', fig.cap="应用主题后的 Beamer 幻灯片", out.width = "90%"}
knitr::include_graphics("examples/beamer-presentation-theme.png", dpi = NA)
```
...
R Markdown 生成的 PowerPoint 幻灯片如图 \@ref(fig:powerpoint-presentation-screenshot)）所示。

```{r powerpoint-presentation-screenshot, fig.align='center', echo=FALSE, fig.cap="PowerPoint 幻灯片", out.width = "90%"}
knitr::include_graphics("examples/powerpoint-presentation-screenshot.png", dpi = NA)
```
...
```{r sharingan-icon, fig.cap="幻灯忍者：写轮眼", echo=FALSE, fig.width=2, fig.height=2,fig.align='center', out.height = "30%"}
knitr::include_graphics("images/xaringan-hex.png", dpi = NA)
```

与其它工具相比，`xaringan` 创建的幻灯片样式更加精美，功能更加丰富，且具有高度的可定制性（图 \@ref(fig:xaringan-screenshot)）。

```{r xaringan-screenshot, fig.cap="`xaringan` 幻灯片的实例",fig.width=6,fig.align='center', echo=FALSE, out.width = "90%"}
knitr::include_graphics("images/xaringan-scrennshot.png", dpi = NA)
```
...
```{r xaringan-new, echo=FALSE, fig.align='center', fig.cap="创建一个新文档", out.width = "90%"}
knitr::include_graphics("images/xaringan-new.png", dpi = NA)
```
...
```{r xaringan-help, fig.width=6,fig.align='center', fig.cap="展示所有快捷键", out.width = "90%"}
knitr::include_graphics("images/xaringan-help.png", dpi = NA)
```

- `p` 进入演讲者模式（可以看见自己写的注释，比如：提醒自己要讲的笑话，图 \@ref(fig:xaringan-review)）。


```{r xaringan-review, echo=FALSE,fig.width=6,fig.align='center', fig.cap="进入演讲者模式", out.width = "90%"}
knitr::include_graphics("images/xaringan-review.png", dpi = NA)
```

- 点击按键 `c` 可以复制幻灯片到新窗口。演讲时可以在自己面前的屏幕上显示演讲者模式，把新窗口中正常的幻灯片拖到大屏幕投影上给观众看（见图 \@ref(fig:xaringan-new-screan)）。

```{r xaringan-new-screan, fig.width=6,fig.align='center', echo=FALSE,fig.cap="复制幻灯片到新窗口", out.width = "90%"}
knitr::include_graphics("images/xaringan-new-screan.png", dpi = NA)
```
...
```{r xaringan-themer-sample-view, echo=FALSE,fig.width=6,fig.align='center', fig.cap="xaringanthemer 包提供的主题示例", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-sample-view.png", dpi = NA)
```
...
```{r xaringan-themer-ggplot, echo=FALSE, fig.width=6, fig.align='center', fig.cap="xaringanthemer 包提供的 ggplot2 色板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-ggplot.png", dpi = NA)
```
...
```{r xaringanthemr-autocomplet, fig.cap="xaringanthemer 支持自动补全功能", echo = FALSE}
if (knitr::is_html_output()) {
  knitr::include_graphics("https://raw.githubusercontent.com/gadenbuie/xaringanthemer/assets/example-rstudio-completion.gif", dpi = NA)
}
```
...
```{r xaringan-themer-demo1, fig.width=6, fig.align='center', echo=FALSE, fig.cap="Moon Light 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo1.png", dpi = NA)
```
...
```{r xaringan-themer-demo2, fig.width=6,fig.align='center', echo=FALSE, fig.cap="Mono Dark 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo2.png", dpi = NA)
```
...
```{r xaringan-themer-demo3,fig.width=6,fig.align='center', echo=FALSE, fig.cap="Mono Accent 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo3.png", dpi = NA)
```
...
```{r xaringan-themer-demo4, fig.width=6,fig.align='center',echo=FALSE, fig.cap="Mono Accent Inverse 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo4.png", dpi = NA)
```
...
```{r xaringan-themer-demo5,fig.width=6,fig.align='center', echo=FALSE, fig.cap="Duo 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo5.png", dpi = NA)
```
...
```{r xaringan-themer-demo6,fig.width=6,fig.align='center', echo=FALSE, fig.cap="Solarized Light 模板", out.width = "90%"}
knitr::include_graphics("images/xaringan-themer-demo6.png", dpi = NA)
```
...
knitr::include_graphics("images/xaringan-extra-1.png", dpi = NA)
```
...
```markdown
## .can-edit.key-firstSlideTitle[可以在此修改幻灯片标题并将修改后的标题保存到当前会话中]
```
...
knitr::include_graphics("images/xaringan-extra-2.png", dpi = NA)
```
...
```{r xaringan-extra-3, echo=FALSE,fig.width=6,fig.align='center', fig.cap="给幻灯片添加全局 logo", out.width = "90%"}
knitr::include_graphics("images/xaringan-extra-3.png", dpi = NA)
```
...
```{r xaringan-extra-4, echo=FALSE,fig.width=6,fig.align='center', fig.cap="添加网络摄像头进行实时视频", out.width = "90%"}
knitr::include_graphics("images/xaringan-extra-4.png", dpi = NA)
```
...
knitr::include_graphics("images/word-new1.png", dpi = NA)
```
...
```{r, word-template-1, echo=FALSE,fig.width=6,fig.align='center', fig.cap='查找特定文档元素的样式。', out.width = "90%"}
...
```{r, word-template-2, echo=FALSE, fig.cap=' 修改 Word 文档中元素的样式。', out.width='90%'}
...
```{r, word-table, echo=FALSE, fig.cap='修改 Word 文档中的表格样式。',fig.width=6,fig.align='center', out.width = "90%"}
...
第 \@ref(yaml) 节介绍了 R Markdown 中输出格式的本质，每一个输出格式都是一个函数。`html_document` 或 `pdf_documnet` 等格式分别对应了 rmarkdown 包中的函数 `rmarkdown::html_document` 和 `rmarkdown::pdf_document`。在 YAML 元数据中指定输出格式的时候，实际上是在设置相应函数的参数。

因此，可以通过创建新的函数来创建新的文档格式。同时，为了方便新格式的使用和分发，人们还会经常开发一个对应的 R 包，随后就可以通过 `output: mypackage::myformat` 来使用新的文档格式。

[^rmarkdown-new]: 这一部分的内容初学者可以安心的选择跳过，不了解这些内容并不会对 R Markdown 的日常使用造成任何困扰。

### 从现有文档格式衍生新格式 {#new-existing}

创建新的文档格式最简单的办法就是基于现有的文档格式衍生出新的文档格式。**rmarkdown**内嵌的文档格式都是高度可定制的，包含众多可用参数。只需要为这些参数赋值，就可以创建出一个新的文档格式出来。

下面的例子针对季度报告创建了一个新格式 `quarterly_report`。这个函数是基于 **rmarkdown** 的内嵌函数 `html_document` 工作的。
...
值得一提的是，第三项和第四项功能依赖于软件包提供的外部文件。也就是说，在 `quarterly_report` 函数中，分别获取了 `"mypackage"` 的软件包中携带的 CSS 文件和一个 HTML 文档，然后分别将 CSS 定义传递给了 `html_document` 函数的 `css` 和 `includes` 参数。后者使用了一个 `rmarkdown::includes()` 函数，从而将这个 HTML 文档（片段）作为文档 `before_body` 的内容嵌入到使用该格式的文档中去。

### 自定义输出格式 {#new-new}

在基于现有格式衍生新输出格式之外，也可以从底层出发创建出一个完全的新格式。

为此需要了解文档格式的底层工作原理，简单来说 R Markdown 的输出格式由下列几个部分构成：
...
可以使用 **rmarkdown** 软件包中的函数 `output_format()` 来创建一个新的输出格式。下面是一个例子：
...
在这个函数中，**knitr** 和 Pandoc 的参数分别由两个参数指定，其内容可以相当的复杂（参见 `?rmarkdown::knitr_options` 和 `?rmarkdown::pandoc_options` 了解更多）。而 `clean_supporting` 则控制是否清理文档编译过程中使用的临时文件。

`output_format()` 函数中还有一个 `base_format` 参数用来指定一个原有的输出格式，并在此基础上进行配置（相当于衍生格式）。

如果有其它的一些支持文件无法通过 `includes` 参数指定，那么还需要使用 `output_format()`中的其它参数来帮助处理这些文件（例如，使用 `intermediates_generator` 来将它们拷贝到文档的输出目录中去）。

学习创建一个新的文档输出格式最好的方法可能是阅读原有格式的源代码，例如从 **rmarkdown** 的两个基础格式 `html_document` 和 `pdf_document` 开始，其源代码的位置在 <https://github.com/rstudio/rmarkdown/tree/master/R>。如果新格式需要用到 Pandoc 模板，则请参见 \@ref(template-pandoc) 及 Pandoc 的使用手册。

### 使用新的文档输出格式 {#new-using}

R 包是 R Markdown 新格式最常见的分发方式。假设 `quarterly_report` 函数定义在名为 **mypackage** 的 R 包中，则使用方法如下：
...
其中，`toc: true` 对应了 `quarterly_report(toc = TRUE)`。
