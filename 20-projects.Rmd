...
函数`knit_child()`返回已编译输出的字符向量，我们可以使用`cat()`和快选项`results = 'asis'`\index{chunk option!results}来将其写回主文档。
...
你可以通过很多设置来自定义一个**bookdown**项目。关于**bookdown**的更全面的概述，可以查看 _R Markdown Definitive Guide_的第十八章[“Creating New Formats”](https://bookdown.org/yihui/rmarkdown/new-formats.html) [@rmarkdown2018]。完整文档请参见书籍 [**bookdown**](https://bookdown.org/yihui/bookdown/) [@bookdown2016]。
