...
后文经常谈到的 "R Markdown 如何如何" 泛指 rmarkdown 包及一系列相关工具的功能与使用，如果特指 `rmarkdown` 包的功能或函数，则用小写字母说明。
...
安装完成后，我们可以开始创建第一个 R Markdown 文档。在 RStudio 中选择 `File --> New File --> R Markdown`, 弹出图 \@ref(fig:create-rmd-prompt) 所示的提示框。

```{r create-rmd-prompt, echo = FALSE}
knitr::include_graphics("images/02-first-rmd-prompt.png")
```

在这里，我们可以提前设定输出文档的标题，作者和格式。这里我们保留默认设置，点击 OK 即可。
