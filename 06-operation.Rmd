...
```{r, magick-logo, dev='png', fig.retina=1, fig.process=add_logo, magick.gravity = 'northeast', fig.cap='通过块选项 fig.process 来给一幅图添加 R logo。', out.width = "90%"}
par(mar = c(4, 4, .1, .1))
...
```{r, dev-tikz, echo=FALSE, tidy=FALSE, fig.cap='通过 tikz  设备呈现的图像。', fig.align='center', cache=TRUE, out.width = "90%"}
...
```{r, low-plots, fig.cap=c('cars 数据的点图', '在现有的点图中添加一条回归线'), fig.keep='low', fig.show = "hold", fig.align = "left", out.width = "49%"}
par(mar = c(4, 4, .1, .1))
plot(cars)
...
本节重点介绍 Pyhton、Shell、SAS 以及 Stata 等语言引擎，如果读者其他语言引擎感兴趣，例如：SQL, Rcpp, Stan, JavaScript, Julia, C，和 Fortran 等，可参见书籍
...
```{r reticulate-intro, echo = FALSE, fig.cap = "R 与 Python 代码块的交互", out.width = "90%"}
...
```{r, hook-scroll, echo=FALSE, fig.cap='一个可滚动文本输出的例子，其高度通过块选项`max.height`来给定', out.width = "90%"}
...
[^html5]: HTML5 是 HTML 的最新标准，为原始 HTML 增加了很多功能，极大地提升了用户的使用体验。如果想更多地了解二者的差异，可以参见 https://www.hostinger.com/tutorials/difference-between-html-and-html5#HTML_vs_HTML5_-_Comparison。
...
```{r hook-html5, echo=FALSE, fig.cap="一个在HTML5 `figure` 标签中的图像", out.width = "90%"}
...
但 **knitr** 的缓存最大的缺点是缓存可能会在不经意间失效，因为影响 knitr 判断是否应该重新计算的因素很多。例如，任何代码块块选项的改变都会使缓存失效[^2]，但有些块选项可能与计算无关。例如在下面的代码块中，改变块选项 `fig.width = 6` 到 `fig.width = 10` 不会影响结果，因为该代码块并不涉及生成图片，但在 knitr 眼中这个代码块的缓存已经过期了，需要重新运行。
...
上述代码块使用了块选项 `fig.show='hide'`\index{chunk option!fig.show} 来把图像暂时隐藏起来。然后在另一段中，通过函数 `knitr::fig_chunk()`\index{knitr!fig\_chunk()} 得到该文件的路径，例如 `test_files/figure-html/cars-plot-1.png`。使用者需要将块标签和图像设备名称传递给 `fig_chunk()`，以便它更好地得到图像文件路径。
