...
目前，大多数非 R 语言的代码块都是独立执行的。例如，同一文档中的所有 `bash` 代码块都在各自的会话中单独执行，因此后面的 `bash` 代码块不能使用在先前 `bash` 代码块中创建的变量，更改后的工作目录（通过 `cd` ）在其他 bash 代码块中不会一直存在。只有 R、Python 和 Julia 代码块在同一个会话中执行。并且 R 会话和 Python 会话是两个不同的会话，但是可以从另一个会话访问或操作一个会话的对象 (详见 <https://bookdown.org/yihui/rmarkdown-cookbook/eng-python.html#eng-python>)。
...
使用 `knitr::knit_engines$set()` 注册一个自定义语言引擎。它接受一个函数作为输入，例如：
...
`upper` 引擎首先将代码向量合成一个完整的字符串，并用 `toupper()` 函数把所有字符转换为大写。注意 `toupper()` 仅在 chunk 选项 `eval = TRUE`\index{chunk option!eval} 时才应用，否则返回原始字符串。

以上已经向读者们展示了如何在引擎函数中使用 `eval` 之类的块选项。类似地，考虑在函数体中添加 `if (options$results == 'hide') return()` 来隐藏 chunk 选项时的输出 `results = 'hide'`\index{chunk option!results} 。下面是一个使用 `upper` 引擎的例子：
...
下面的例子实现了一个基础的 Python 引擎。^[实际工作中读者应使用 R Markdown 内置的 Python 引擎]。首先获得代码块中完整的 Python 代码，随后使用 `system2()` 函数在命令行调用 Python 解释器，最后格式化输出。
...
`knitr::engine_output()`\index{knitr!engine\_output()} 常用于返回运行结果，它会自动响应如 `echo = FALSE` 和 `results = 'hide'` 等块选项，所以这里不需要额外的 `if` 语句。
...
下面是一个 Asymptote 与 R 交互的例子，首先 R 中的数据保存到 CSV 文件中：
...
R Markdown 在底层使用 **sass** [@R-sass] \index{R package!sass} 包中的 `sass::sass()` 函数将 Sass 编译为常规 CSS。块选项 `engine.opts` 可以定制 CSS 代码的输出样式，例如：`engine.opts = list(style = "expanded")`。默认的样式是 "compressed"。请参阅帮助页面 `?sass::sass_options` 了解更多 `output_style` 可用的参数。
...
在 R Markdown 中，钩子函数 (hook function) 负责将代码块的输出转换为最终文档里的视觉元素。**knitr** 运行完代码块后将结果存储为字符向量，钩子函数将这个字符向量加工为新的字符向量并返回到文档中，这就是用户最终看到的结果。考虑下面的代码块：
...
**knitr** 运行该段代码后，得到两个字符串：源代码 `"1 + 1"` 和文本输出 `"[1] 2"`。现在需要两个钩子函数把它们转换为输出元素，一个负责源代码，一个负责文本输出。对于源代码，R Markdown 默认调用 **knitr** 中 `source` 钩子，它的简化版如下：
...
可以看到，钩子函数在实现上只是普通的 R 函数。上面的钩子函数将源代码转换为 Markdown 代码块，即以 ` ``` ` 开头和结尾，并添加语言标记。
...
`source` 和 `output` 钩子的实现比上面的两个函数更复杂，但背后的思想是一样的。读者可以通过在 `knit_hooks`\index{knitr!knit\_hooks} 中查看钩子函数的源代码，例如：
