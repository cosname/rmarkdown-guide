...
```{r word-new1, fig.width=6, echo=FALSE, fig.cap="新建 R Markdown 时可选的 word 格式"}
...
有时，查找元素的样式名称并不简单。可能有多种样式应用于同一个元素，并且你只会在样式列表中看到其中一个突出显示。可能需要一些猜测和在线搜索才能确定你要修改的实际样式。 比如你要点击“管理样式”按钮 （图\@ref(fig:word-template-1) 中样式列表底部从左到右的第三个按钮），滚动一个大数字 在找到“表格”样式之前的样式名称 （参见图 \@ref(fig:word-table)）。 然后你可以为你的表格修改此样式（例如，添加边框）。
