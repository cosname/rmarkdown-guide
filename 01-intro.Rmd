...
<<<<<<< HEAD
从诞生开始，R 便是一门致力于交互式数据分析的语言，R 用户便时常有着制作数据分析报告的需求，这也是 R Markdown 最常见的用途。它支持 HTML, PDF, Word, EPUB 等多种输出格式，以及与之配套开箱即用的主题系统。用户可以将创作重心放在内容上，写作完成后一键输出不同格式，让 R Markdown 的自动化的编译机制代劳形式上的调整。例如，只需要几行代码，同一个 R Markdown 文档就可以生成图 \@ref(fig:three-output) 中 HTML，PDF 和 Word 三种格式的文档。


```{r three-output, fig.show = "hold", fig.align='default', out.width = "33%", echo = FALSE, fig.cap = "同一个 R Markdown 文档可以生成多种格式的输出"}
=======
从诞生开始，R 便是一门致力于交互式数据分析的语言，R 用户便时常有着制作数据分析报告的需求，这也是 R Markdown 最常见的用途。它支持 HTML, PDF, Word, EPUB 等多种输出格式，以及与之配套开箱即用的主题系统。用户可以将创作重心放在内容上，写作完成后一键输出不同格式，让 R Markdown 的自动化的编译机制代劳形式上的调整。例如，只需要几行代码，同一个 R Markdown 文档就可以生成图 \@ref(fig:three-output) HTML，PDF 和 Word 三种格式的文档。


```{r three-output, fig.show = "hold", fig.align='default', out.width = "33%", echo = FALSE, fig.cap = ""}
>>>>>>> master
