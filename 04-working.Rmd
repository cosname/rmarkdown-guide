...
如果文档中不涉及动态网页内容，那么把 R Markdown 文档输出为 Markdown 是非常推荐的，因为这样读者可以更加方便的看到相应的结果（GitHub 网站可以渲染 Markdown）。

```{r eval=FALSE}
rmarkdown::render("analysis.Rmd",md_document(variant = "markdown_github"))
```
